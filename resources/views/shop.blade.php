            ...
            @foreach($prod as $product)
                <div class="justify-center border ml-4 text-lg text-gray-600 leading-7 font-semibold"><img class="mx-auto" src="{{ url( "storage/$product->image") }}"> {{ $product->product_name }}
                    <div class="mt-2 text-sm text-gray-500">
                        ...
                        <p>Price: £{{ $product->price }} <button class = "w-16 inline-flex items-center justify-center p-2 rounded-md bg-blue-300 text-gray-800 hover:text-gray-800 hover:bg-red-500 focus:outline-none focus:bg-blue-900 focus:text-gray-500 transition duration-150 ease-in-out">Buy</button> </p>
